...
label_coverage_var <- function(var, var_name = NULL, labels_df = NULL) {
  non_na <- var[!is.na(var)]
  if (is.null(labels_df) || is.null(var_name)) return(NA_real_)
  labels_var <- labels_df %>% dplyr::filter(var == var_name & value != ">")
  if (length(non_na) == 0) return(NA_real_)
  df_values <- data.frame(var = var_name, value = unique(non_na), stringsAsFactors = FALSE)
...
topk_ratios_var <- function(var, topk = c(2,3)) {
  ...
  counts <- table(non_na)
  probs <- if (sum(counts) > 0) counts / sum(counts) else numeric(0)
  if (length(probs) == 0) return(setNames(rep(NA_real_, length(topk)), paste0("top", topk, "_ratio")))
  p_sorted <- sort(probs, decreasing = TRUE)
  ratios <- vapply(topk, function(k) sum(p_sorted[seq_len(min(k, length(p_sorted)))]), numeric(1))
